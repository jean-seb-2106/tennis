...
#tennis base de données
data <- readxl::read_xlsx("C:/Users/XX9JQZ/Downloads/archive(3)/atp_mens_tour/2023.xlsx")
data <- data %>% 
  mutate_at(vars(WRank,LRank,WPts,LPts),as.numeric) #transfo en numérique

recette_data <- recipe(~., data = data) %>%
  step_naomit(WRank, LRank,B365W,PSW,MaxW,AvgW) %>% 
  step_rm(W1,L1,W2,L2,W3,L3,W4,L4,W5,L5,Wsets,Lsets)#enlever les individus avec des valeurs manquantes
data <- prep(recette_data) %>%
  bake(new_data = NULL) 
rm(recette_data)

data <- data %>% mutate_if(target = )
